...
def main():
    ...
    for chunk in CHUNKS:
        ...
        extraction = extract(chunk_text=chunk['text'], chunk_id=chunk['id'], mock_mode=False)
        builder.add_extraction(extraction)
        
    print("\nExtraction complete. Building outputs...")
    
 
    graph_json = builder.build_graph()
    with open("output_graph.json", "w") as f:
        f.write(graph_json)
    print("Saved -> output_graph.json")
    

    mermaid_text = builder.render_mermaid()
    with open("output_mermaid.md", "w") as f:
        f.write("```mermaid\n")
        f.write(mermaid_text)
        f.write("\n```")
    print("Saved -> output_mermaid.md")

    
    print("\n--- Testing Retrieval Module ---")
    query = "instamart"
    chunks = builder.retrive_chunks(query)
    print(f"Query: '{query}' -> Found in Chunks: {chunks}")
